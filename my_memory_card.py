...
questions_list = [] 
questions_list.append(Question('Benua terbesar di dunia?', 'Asia', 'Afrika', 'Australia', 'Eropa'))
questions_list.append(Question('Indonesia merupakan bagian dari Asia?', 'Tenggara', 'Selatan', 'Timur', 'Tengah'))
questions_list.append(Question('Negara yang termasuk di benua Eropa (KECUALI)?', 'Hong Kong', 'Itali', 'Ukraina', 'Jerman'))


app = QApplication([])


btn_OK = QPushButton('Jawab') # answer button
lb_Question = QLabel('The most difficult question in the world!') # question text


RadioGroupBox = QGroupBox("Jawaban") # on-screen group for radio buttons with answers
...
def show_result():
    ...
    RadioGroupBox.hide()
    AnsGroupBox.show()
    btn_OK.setText('Soal Selanjutnya')
...
window.setWindowTitle('Memory Card (Geografis)')
